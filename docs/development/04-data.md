...
| `/home/ubuntu/researchbuddy/backend/projects/` | `/backend/projects/` | 每个研究项目的裸 git 仓库 |
...
# 把数据库拉到本地
scp ubuntu@43.163.8.22:/home/ubuntu/researchbuddy/backend/db.sqlite3 \
...
新服务器首次部署时，`backend/db.sqlite3` 不存在，Docker bind mount 会把挂载点创建成目录（而不是文件），导致 SQLite 无法打开。解决方法：

```bash
ssh ubuntu@43.163.8.22 "
  cd /home/ubuntu/researchbuddy
  sudo docker compose -f docker-compose.prod.yml stop backend
  sudo rmdir backend/db.sqlite3    # 如果被创建成了目录
